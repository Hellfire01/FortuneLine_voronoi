...
public static class getVoronoï {
    ...
    /// <returns></returns>
    private static Node get_or_assign_node_in_dictionnary(ref Dictionary<Vector2, Node> vertex_dictionnary, ref Seed seed, Vector2 pos, ref PreMetaData metaData) {
        Node buffer = null;
        try {
            buffer = vertex_dictionnary[pos];
        } catch (KeyNotFoundException e) {
            bool is_out_of_bounds = false;
            if (pos.x < 0 || pos.x > seed.mapRelativeSize || pos.y < 0 || pos.y > seed.mapRelativeSize) {
                is_out_of_bounds = true;
            }
            buffer = new Node(pos, is_out_of_bounds);
            buffer.setValues((int)(pos.x / seed.squareRelativeSize), (int)(pos.y / seed.squareRelativeSize));
    ...
    private static void extract_data_from_graph_edge_list(List<GraphEdge> to_translate, ref Seed seed, ref PreMetaData metaData) {
        ...
        foreach (GraphEdge edge in to_translate) {
            ...
            Node buffer1 = get_or_assign_node_in_dictionnary(ref vertex_dictionnary, ref seed, pos1, ref metaData);
            Node buffer2 = get_or_assign_node_in_dictionnary(ref vertex_dictionnary, ref seed, pos2, ref metaData);
    ...
    /// <param name="seed"></param>
    /// <param name="metaData"></param>
    public static void getVoronoi(ref Seed seed, ref PreMetaData metaData) {
        ...
        List<GraphEdge> to_translate =  voronoi.generateVoronoi(translated_sites, xVal, yVal, 0, seed.mapRelativeSize - 1, 0, seed.mapRelativeSize - 1);
        extract_data_from_graph_edge_list(to_translate, ref seed, ref metaData);

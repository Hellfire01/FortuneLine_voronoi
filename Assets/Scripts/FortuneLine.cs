...
namespace FortuneLine {
    ...
    public class Voronoi {
        ...
        public List<GraphEdge> generateVoronoi(Site[] translated_sites, double[] xValuesIn, double[] yValuesIn, double minX, double maxX, double minY, double maxY) {
            sort(translated_sites, xValuesIn, yValuesIn, xValuesIn.Length);
        ...
        private void sort(Site[] translated_sites, double[] xValuesIn, double[] yValuesIn, int count) {
            sites = null;
            ...
            sortNode(translated_sites, xValues, yValues, count);
        }
        ...
        private void sortNode(Site[] translated_sites, double[] xValues, double[] yValues, int numPoints) {
            nsites = numPoints;
        ...
        private Edge bisect(Site s1, Site s2) {
            ...
            newedge.cellcores[0] = s1;
            newedge.cellcores[1] = s2;

            newedge.vertices[0] = null;
            newedge.vertices[1] = null;
        ...
        private Site leftreg(Halfedge he) {
            ...
            return (he.ELpm == LE ? he.ELedge.cellcores[LE] : he.ELedge.cellcores[RE]);
        }
        ...
        private void pushGraphEdge(Site leftSite, Site rightSite, double x1, double y1, double x2, double y2) {
            ...
            newEdge.site_class_1 = leftSite;
            newEdge.site_class_2 = rightSite;
        }
        ...
        private void clip_line(Edge e) {
            ...
            double x1 = e.cellcores[0].coord.x;
            double y1 = e.cellcores[0].coord.y;
            double x2 = e.cellcores[1].coord.x;
            double y2 = e.cellcores[1].coord.y;
            double x = x2 - x1;
            double y = y2 - y1;
            ...
            pushGraphEdge(e.cellcores[0], e.cellcores[1], x1, y1, x2, y2);
        }

        private void endpoint(Edge e, int lr, Site s) {
            e.vertices[lr] = s;
            if (e.vertices[RE - lr] == null)
                return;
            clip_line(e);
        }
        ...
        private bool right_of(Halfedge el, Point p) {
            ...
            if (e.a == 1.0) {
                ...
                if (!fast) {
                    dxs = topsite.coord.x - (e.cellcores[0]).coord.x;
        ...
        private Site rightreg(Halfedge he) {
            ...
            return (he.ELpm == LE ? he.ELedge.cellcores[RE] : he.ELedge.cellcores[LE]);
        }
        ...
        private Site intersect(Halfedge el1, Halfedge el2) {
            ...
            if (e1.cellcores[1] == e2.cellcores[1])
                return null;
            ...
            if ((e1.cellcores[1].coord.y < e2.cellcores[1].coord.y)
                || (e1.cellcores[1].coord.y == e2.cellcores[1].coord.y && e1.cellcores[1].coord.x < e2.cellcores[1].coord.x)) {
                el = el1;
                e = e1;
            } else {
                el = el2;
            ...
            right_of_site = xint >= e.cellcores[1].coord.x;
            if ((right_of_site && el.ELpm == LE)
        ...
        private bool voronoi_bd() {
            ...
            Console.WriteLine();
            Console.WriteLine("========================================");
            Console.WriteLine(this.allEdges.Count);
            foreach (GraphEdge gr in this.allEdges) {
                Console.WriteLine();
                Console.WriteLine("CellCores : " + Math.Round(gr.site_class_1.coord.x) + ", " + Math.Round(gr.site_class_1.coord.y) + " / " +
                    Math.Round(gr.site_class_2.coord.x) + ", " + Math.Round(gr.site_class_2.coord.y));
                Console.WriteLine("Vertices : " + Math.Round(gr.x1) + ", " + Math.Round(gr.y1) + " / " + Math.Round(gr.x2) + ", " + Math.Round(gr.y2));
            }
